...
echo "🔧 WhatsApp Engine - Complete Fix Script (with sudo)"
echo "==================================================="
echo ""

# Step 1: Fix server-optimized.js with sudo
echo "📝 Step 1: Memperbaiki server-optimized.js..."
if [ -f "fix-server-sudo.js" ]; then
    sudo node fix-server-sudo.js
    if [ $? -eq 0 ]; then
    ...
    echo "❌ File fix-server-sudo.js tidak ditemukan!"
    exit 1
fi
...
echo "🔐 Step 5: Memperbaiki permission file..."
sudo chown -R $USER:$USER .
sudo chmod -R 755 .
echo "✅ Permission file sudah diperbaiki!"
...
echo "   ✅ Permission file diperbaiki"
echo ""
