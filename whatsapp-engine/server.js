...
// Read PORT from environment variable (Zeabur sets this automatically)
// If PORT is not set, fallback to 3000
...
console.log(`   PORT: ${PORT} (from ${process.env.PORT ? 'environment variable' : 'default'})`);
...
// Helper function to kill process using port
function killProcessOnPort(port) {
    return new Promise((resolve) => {
        console.log(`🔍 Mencoba mencari dan kill process di port ${port}...`);
        
        // Try different commands based on OS
        const commands = [
            `lsof -ti :${port} | xargs kill -9 2>/dev/null`, // Linux/Mac
            `fuser -k ${port}/tcp 2>/dev/null`, // Linux alternative
            `kill -9 $(lsof -ti :${port}) 2>/dev/null`, // Linux/Mac alternative
        ];
        
        let commandIndex = 0;
        const tryNextCommand = () => {
            if (commandIndex >= commands.length) {
                console.log(`⚠️  Tidak bisa kill process secara otomatis.`);
                resolve(false);
                return;
            }
            ...
            exec(commands[commandIndex], (error, stdout, stderr) => {
                if (error) {
                    ...
                    console.log(`✅ Process di port ${port} berhasil di-kill`);
                    resolve(true);
                }
            });
        };
...
function startServer(retryCount = 0, maxRetries = 1) {
    let server;
    ...
    try {
        ...
        server.on('error', async (error) => {
            ...
            if (error.code === 'EADDRINUSE') {
                ...
                if (retryCount < maxRetries) {
                    console.log(`🔄 Mencoba kill process yang menggunakan port ${PORT}...`);
                    const killed = await killProcessOnPort(PORT);
                    
                    if (killed) {
                        console.log(`⏳ Menunggu 2 detik sebelum retry...`);
                        await new Promise(resolve => setTimeout(resolve, 2000));
                        console.log(`🔄 Retry start server (attempt ${retryCount + 1}/${maxRetries})...\n`);
                        startServer(retryCount + 1, maxRetries);
                        return;
                    }
                }
                
                console.error(`   💡 Solusi Manual:`);
                console.error(`   1. Cek process yang menggunakan port ${PORT}:`);
                console.error(`      lsof -i :${PORT}`);
                console.error(`      atau`);
                console.error(`      netstat -tulpn | grep :${PORT}`);
                console.error(`   2. Kill process secara manual:`);
                console.error(`      kill -9 $(lsof -ti :${PORT})`);
                console.error(`      atau`);
                console.error(`      fuser -k ${PORT}/tcp`);
                console.error(`   3. Atau restart service di Zeabur Dashboard`);
        ...
        if (error.code === 'EADDRINUSE') {
            ...
            (async () => {
                ...
                console.error(`   💡 Solusi Manual:`);
                console.error(`   1. Kill process: kill -9 $(lsof -ti :${PORT})`);
                console.error(`   2. Atau restart service di Zeabur Dashboard`);
                process.exit(1);
            })();
        } else {
